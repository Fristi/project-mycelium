...
object DoobieStationMeasurementRepository extends StationMeasurementRepository[ConnectionIO] {
  ...
  override def avg(stationId: UUID, period: MeasurementPeriod): ConnectionIO[List[StationMeasurement]] = {
    ...
    fr"SELECT $timeBucket AS bucket, avg(battery_voltage) as battery_voltage, avg(temperature) as temperature, avg(humidity) as humidity, avg(lux) as lux, avg(soil_pf) as soil_pf, avg(tank_pf) as tank_pf FROM station_measurements GROUP BY bucket ORDER BY bucket ASC LIMIT $limit".query[StationMeasurement].to[List]
  }
}
